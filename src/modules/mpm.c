...
{
	uint16_t flag = FIRST_CROSSING_NOT_PASSED;
	uint16_t valid_peak_flag = VALID_PEAK_NOT_FOUND;
	...
	for (uint16_t i = 0; i < BLOCK_SIZE; i++)
    {
       ...
       if (flag == FIRST_CROSSING_PASSED)
       {
       		if (*pSrc > peak_value && *pSrc > PEAK_THRESHOLD)
       		{
				peak_value = *pSrc;
              	*tau = i;
                valid_peak_flag = VALID_PEAK_FOUND;

       		} else if (valid_peak_flag == VALID_PEAK_FOUND)
       			return;
       }
       pSrc++;
    }
